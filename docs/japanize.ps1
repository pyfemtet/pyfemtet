...
uv run sphinx-apidoc -f -o $SOURCE_DIR\modules .\pyfemtet

if (-not (test-path $LOCALE_DIR)) {mkdir $LOCALE_DIR}

uv run sphinx-build -b gettext $SOURCE_DIR $GETTEXT_DIR
uv run sphinx-intl update -p $GETTEXT_DIR -l ja_JP -d $LOCALE_DIR --line-width=-1
