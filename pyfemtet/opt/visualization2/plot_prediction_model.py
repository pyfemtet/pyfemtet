...
class PredictionModelPlotter:

    # noinspection PyAttributeOutsideInit
    def fit(self, history, df=None) -> None:
        """

        Notes:
            Specify df if this method is called in dash callback
    ...
    def plot_prediction_model(
        ...
        Args:
            prm_name (str):
            obj_name (str):
            remaining_x (List[float], optional): Input values excluding prm_name_1 and _2.
            prm_name_2 (str, optional): If specified, returns 3d figure.
        ...
        # remaining_x が必要なのに None ではないか確認
        remaining_x = remaining_x or []
        n_additional_prm = len(remaining_x)
        if prm_name_2:
            n_passed_prm = 2
        else:
            n_passed_prm = 1
        assert n_passed_prm + n_additional_prm == len(history.prm_names)

        # determine axis1, axis2 range
        lb1 = df[f'{prm_name}_lower_bound'].min()
        ub1 = df[f'{prm_name}_upper_bound'].max()
        lb1 = df[prm_name].min() if np.isnan(lb1) else lb1
        ub1 = df[prm_name].max() if np.isnan(ub1) else ub1
        if prm_name_2:
        ...
        for prm_name in history.prm_names:
            if prm_name == prm_name:
                tmp_df[prm_name] = xx.ravel()  # shape: (N**2,) or (N,)
        ...
        # ===== create figure =====
        # description: データ点は多次元空間上の点なので、
        # 余剰次元を無視して三次元空間上に投影しても何の意味もない。
        # むしろ混乱するのでやってはいけない。実験的機能として、透明度を設定してみる。
        ...
        if prm_name_2:
            fig = go.Figure(data=go.Scatter3d(
                x=df[prm_name], y=df[prm_name_2], z=df[obj_name],
                mode='markers',
                marker=dict(
                    size=3,
                    color='black',
        ...
        # set opacity by its distance from (super) plain
        ...
        if prm_name_2:
            ...
            # std
            fig.add_trace(go.Surface(z=zz_upper, x=xx, y=yy, showscale=False, opacity=0.3, showlegend=True,
                                     name=Msg.LEGEND_LABEL_PREDICTION_MODEL_STDDEV))
            fig.add_trace(go.Surface(z=zz_lower, x=xx, y=yy, showscale=False, opacity=0.3, showlegend=True,
                                     name=Msg.LEGEND_LABEL_PREDICTION_MODEL_STDDEV))
...
def plot_prediction_model(
        ...
        prm_name: str,
        obj_name: str,
        remaining_x: Optional[list[float]] = None,
        df: Optional[pd.DataFrame] = None,
        prm_name_2: Optional[str] = None
):
    plotter = PredictionModelPlotter()
    plotter.fit(history, df)
    return plotter.plot_prediction_model(prm_name, obj_name, remaining_x, prm_name_2)
