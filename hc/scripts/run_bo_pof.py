...
if __name__ == '__main__':
    ...
    for initial_point in problem.initial_points:
        random_manager.sampling(initial_point)
        break
    ...
    while len(random_manager.x_feasible) < 5:
        ...
        print(
            ...
            f'{
                (
                    float(len(random_manager.x_feasible))/float(len(random_manager.x))
                )*100
                :.0f}%'
            f')'
        )
    ...
    for i in tqdm(range(50), 'bayesian pof sampler'):
